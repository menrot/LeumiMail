...
def parse_HTML_notice(file, accounts, workingDir):
    origDir = os.getcwd()
    os.chdir(workingDir)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("usage parse_rename HTML file, workinDir")
        exit(1)
    else:
        Accounts = Table()
        csv_fp = open('ListOfAccounts.csv', 'rt')
        Accounts.populate_Table(csv_fp)
        cust_name, datePrefix = parse_HTML_notice(sys.argv[1], Accounts, sys.argv[2])
